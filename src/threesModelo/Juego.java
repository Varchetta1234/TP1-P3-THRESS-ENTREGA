...
public class Juego {
	...
	public Direccion sugerirProximaJugada() {
		Direccion mejorDireccion = null;
		int maxPuntaje = -1;
		int maxCeldasVacias = -1;

		for (Direccion direccion : Direccion.values()) {
			// 1. Guardar el estado real del tablero
			int[][] estadoOriginal = tablero.obtenerCopia();
			
			// 2. Simular el movimiento (usamos el método interno que no agrega fichas nuevas)
			boolean movimientoValido = moverTodasLasFichas(direccion);

			// 3. Evaluar si conviene este movimiento
			if (movimientoValido) {
				int puntajeSimulado = obtenerPuntaje();
				int celdasVaciasSimuladas = tablero.obtenerCeldasVacias().size();

				// Elegir si da más puntaje, o si da el mismo puntaje pero libera más celdas
				if (puntajeSimulado > maxPuntaje || (puntajeSimulado == maxPuntaje && celdasVaciasSimuladas > maxCeldasVacias)) {
					maxPuntaje = puntajeSimulado;
					maxCeldasVacias = celdasVaciasSimuladas;
					mejorDireccion = direccion;
				}
			}

			// 4. Deshacer la simulación restaurando el tablero a su estado original
			for (int fila = 0; fila < Tablero.TAMANIO; fila++) {
				for (int columna = 0; columna < Tablero.TAMANIO; columna++) {
					tablero.asignarValor(fila, columna, estadoOriginal[fila][columna]);
				}
			}
		}
