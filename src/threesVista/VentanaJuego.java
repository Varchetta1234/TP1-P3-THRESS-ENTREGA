...
public class VentanaJuego extends JFrame {
	...
	private JButton botonSugerencia;
	...
	private void agregarTablero() {
		...
		panelTablero.setLayout(new GridLayout(Tablero.TAMANIO, Tablero.TAMANIO, 8, 8));
		panelContenido.add(panelTablero);

		etiquetasDeLasCeldas = new JLabel[Tablero.TAMANIO][Tablero.TAMANIO];
	...
	private void agregarPiePagina() {
		...
		botonSugerencia = new JButton("Sugerir Jugada");
		botonSugerencia.setToolTipText("Muestra el mejor movimiento para sumar puntos");
		botonSugerencia.setFont(new Font("Arial", Font.BOLD, 14));
		botonSugerencia.setBounds(180, 520, 150, 35);
		botonSugerencia.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent evento) {
				mostrarSugerencia();
			}
		});
		panelContenido.add(botonSugerencia);
	...
	private void mostrarSugerencia() {
		...
		Direccion sugerencia = juego.sugerirProximaJugada();

		if (sugerencia != null) {
			JOptionPane.showMessageDialog(
					this,
					"Te sugiero mover hacia: " + sugerencia.name(),
					"Sugerencia",
					JOptionPane.INFORMATION_MESSAGE
			);
